...
import mnemosyne.llm_authoring as llm_authoring
from mnemosyne.chat_models import providers as chat_providers
from mnemosyne.runtime.dependencies import build_install_command
from mnemosyne.runtime.judge import judge_code
from mnemosyne.problem_authoring import PROBLEM_TEMPLATE, format_problem_json, validate_problem_spec
...
def main() -> int:
    checks = [
        ...
        ("OpenAI-compatible payload injects prompt contract and falls back", check_openai_compatible_payload_shape),
...
def check_authoring_repair() -> str:
    ...
    draft["statement"] = (
        "# Repair NumPy Example\n\n"
        "Return `x` as a floating point NumPy-compatible array.\n\n"
        "## Input / Output\n\n"
        "- `x`: `list[float]`, a JSON list of numeric values.\n"
        "- Return: `numpy.ndarray`, the same values as floats."
    )
...
def check_authoring_newline_import_repair() -> str:
    ...
    draft["statement"] = (
        "# Repair\\n\\n"
        "Compute with $\\nabla f$ notation and return the values as floats.\\n\\n"
        "## Input / Output\\n\\n"
        "- `x`: `list[float]`, the input values.\\n"
        "- Return: `numpy.ndarray`, the values converted to floats."
    )
...
def check_authoring_test_alias_repair() -> str:
    ...
    word_count["statement"] = (
        ...
        "Count how many times each word appears in `words` and return the frequency map.\n\n"
        "## Input / Output\n\n"
        "- `words`: `list[str]`, the sequence of words to count.\n"
        "- Return: `dict[str, int]`, mapping each word to its count."
    )
...
def check_authoring_accepts_short_inline_equations() -> str:
    ...
    draft["function_name"] = "inline_full_equation"
    draft["statement"] = (
        "Compute the loss $L(x) = x^2 + 1$ for every number in `nums` and return the total. "
        "Short symbols like $x$ are fine.\n\n"
        "## Input / Output\n\n"
        "- `nums`: `list[int]`, the input values.\n"
        "- Return: `int`, the summed loss."
    )
    draft["starter_code"] = "def inline_full_equation(nums: list[int]) -> int:\n    pass\n"
    draft["reference_solution"] = "def inline_full_equation(nums):\n    return sum(x * x + 1 for x in nums)\n"
    draft["visible_tests"] = [{"name": "basic", "args": [[1, 2]], "expected": 7}]
    draft["hidden_tests"] = []
    ...
    long_inline["statement"] = (
        ...
        "(x_i^T \\theta - y_i)^2 + \\lambda \\sum_{j=1}^{n} \\theta_j^2$ for `nums` and return the total.\n\n"
        "## Input / Output\n\n"
        "- `nums`: `list[int]`, the input values.\n"
        "- Return: `int`, the summed loss."
    )
...
def check_unordered_nested_checker() -> str:
    ...
    problem["statement"] = (
        "Group the strings in `strs` so each returned group contains anagrams of the same letters. "
        "Group order does not matter.\n\n"
        "## Input / Output\n\n"
        "- `strs`: `list[str]`, the words to group.\n"
        "- Return: `list[list[str]]`, groups of anagrams."
    )
...
def check_gemini_payload_shape() -> str:
    original = chat_providers._urlopen_json
    seen_payloads = []
    ...
    try:
        chat_providers._urlopen_json = fake_urlopen_json
        ...
        chat_providers._urlopen_json = original
    ...
    try:
        chat_providers._urlopen_json = fake_thinking_budget_error
        text = client.generate_json(
            [{"role": "user", "content": "Return {\"ok\": true}."}],
            {
                "name": "mnemosyne_problem_collection",
    ...
    assert chat_providers._gemini_request_timeout(60, "gemini-2.5-flash", [{"mime_type": "application/pdf"}]) == 180
    assert chat_providers._gemini_request_timeout(60, "gemini-3.1-pro-preview", [{"mime_type": "application/pdf"}]) == 240
...
def check_openai_compatible_payload_shape() -> str:
    original = chat_providers._urlopen_json
    seen_payloads = []
    ...
    try:
        chat_providers._urlopen_json = fake_urlopen_json
        client = llm_authoring.OpenAICompatibleClient(
            api_key="test-key",
            base_url="https://compatible.example/v1",
            model="compatible-json-model",
        )
        text = client.generate_json(
            [{"role": "user", "content": "Create one problem."}],
            {
                "name": "mnemosyne_problem_collection",
                "schema": {
                    "type": "object",
    ...
    assert text == "{\"ok\": true}", text
    assert seen_payloads, "OpenAI-compatible request was not made"
    url, payload, label = seen_payloads[0]
    assert url == "https://compatible.example/v1/chat/completions", url
    assert label == "OpenAI-compatible API", label
    response_format = payload["response_format"]
    assert response_format["type"] == "json_schema", payload
    assert response_format["json_schema"]["strict"] is True, payload
    request_text = payload["messages"][-1]["content"]
    assert "Required JSON contract" in request_text, request_text
    assert "Few-shot examples" in request_text, request_text
    ...
    def fake_schema_error_then_json_object(request, timeout_seconds, label):  # noqa: ANN001 - local monkeypatch.
        payload = json.loads(request.data.decode("utf-8"))
        seen_payloads.append((request.full_url, payload, label))
        if len(seen_payloads) == 1:
            raise llm_authoring.LLMRequestError("response_format json_schema is not supported by this endpoint")
        return {"choices": [{"message": {"content": "{\"ok\": true}"}}]}

    try:
        chat_providers._urlopen_json = fake_schema_error_then_json_object
        client = llm_authoring.OpenAICompatibleClient(
            api_key="test-key",
            base_url="https://compatible.example/v1",
            model="compatible-json-model",
    ...
    assert text == "{\"ok\": true}", text
    assert len(seen_payloads) == 2, seen_payloads
    assert seen_payloads[0][1]["response_format"]["type"] == "json_schema", seen_payloads
    assert seen_payloads[1][1]["response_format"] == {"type": "json_object"}, seen_payloads
    fallback_request_text = seen_payloads[1][1]["messages"][-1]["content"]
    assert "Required JSON contract" in fallback_request_text, fallback_request_text
    assert "Few-shot examples" in fallback_request_text, fallback_request_text
    return "strict json_schema prompt contract, then json_object prompt fallback"
...
def check_llm_repair_hints() -> str:
    errors = [
        ...
        "problems[0] (vec): starter_code parameter `x` is annotated as `np.ndarray`, so arg_types[0] must declare the runtime conversion.",
    ...
    assert "runtime_interface_contract" in codes, report
